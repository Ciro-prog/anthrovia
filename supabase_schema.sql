...
-- Create a policy that allows anyone to update (for now, until auth is set up)
-- WARNING: This allows anyone with the anon key to update the data. 
-- In a real production app, you would restrict this to authenticated users.
create policy "Enable update access for all users" on public.sections
  for update using (true) with check (true);

create policy "Enable insert access for all users" on public.sections
  for insert with check (true);

-- Insert initial data (Hero)
insert into public.sections (id, type, content)
values (
  'hero',
  'hero',
  '{
